...
- **代码审计加固**: 10 HIGH + 14 MED 安全问题全部修复
- **多轮工具调用**: CRITICAL 修复现代 tool_result 格式被丢弃、孤立 tool_calls 被误删
- **连接池调优**: HTTP Transport 自定义, goroutine 泄漏修复, ctx 传播
- **SenseNova 兼容**: 流式/非流式 reasoning 字段回退
- **版本管理修复**: npm 包装器无限循环修复，semver 向前兼容
- **测试体系**: 137+ 新增测试，端到端冒烟测试，Provider 87% 覆盖
- **发布工具**: 全新 release.sh 6 阶段自动发布
